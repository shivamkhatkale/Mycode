...
public class CommonRestUtils {
	...
	public static String BearerToken; //added by Sachin
	FileHandler fileHandlerToResponseFolder = new FileHandler("responseFolder");
	...
	//Below is the Old Post Request method is used for API chaining --> I am commenting 
//	@Then("user triggers a post request with (.*)$")
//	public void triggerPostRequestCommonRest(String requestBody) {
//		
//		ObjectMapper objectMapper = new ObjectMapper();
...
//	}
	...
	@Then
	("user triggers a post request with (.*)$")
...
//	@Given("user sets the bearer token")
//	public static void settingBearerToken(String filePath) {
//		ObjectMapper objectMapper = new ObjectMapper();
//
//		try {
//			// String baseDir = System.getProperty("user.dir");
	...
	public static void settingBearerToken(String bearertokenPath) throws Exception {
		...
		try {
			...
			Response response = SerenityRest.given().header("Content-Type", "application/json")
					.header("dbp-id", stringdbpID).header("dbp-secret", stringdbpSecret)// Add any other headers as
					// needed
					.when().body(jsonString).post(RestAssured.baseURI.trim());
			if (response.statusCode() == 200) {
				ObjectMapper objectMapperBearer = new ObjectMapper();
				String responseBody = response.then().extract().body().asString();
				JsonNode jsonNode = objectMapperBearer.readTree(responseBody);
		...
		else
		{
			...
			try {
				...
				Path configFilePath = Paths.get(TestBase._baseDir,bearertokenPath);
				String jsonString = objectMapper.readTree(new File(configFilePath.toString())).toString();
				System.out.println("Request Body: " + jsonString);
...
//		}
	...
	//changes done in SSA 405 method
	//SSA 405 Method
  ...
  //APY new method
  	...
  	public void sendsPostRequestForAPY(String requestBody) {
  		ObjectMapper objectMapper = new ObjectMapper();
  		try {
  			Path configFilePath = Paths.get(TestBase._baseDir, requestBody);
  			String jsonString = objectMapper.readTree(new File(configFilePath.toString())).toString();
  	...
  	public void sendsGetRequestForAPY(String requestBody) {
  		ObjectMapper objectMapper = new ObjectMapper();
  		try {
  			Path configFilePath = Paths.get(TestBase._baseDir, requestBody);
  			String jsonString = objectMapper.readTree(new File(configFilePath.toString())).toString();
  	...
  	//get the customer account new method general service
  	@Then
  	("user triggers a post request to get customer accounts (.*)$")
  		public void triggerPostRequesttogetCustomerAccount(String requestBody) {
  		    ...
  		    try {
  		       ...
  		       String responseBody = response.then().extract().body().asString();
  		 			Serenity.setSessionVariable("response").to(response);
  		 			System.out.println("Response Body triggerPostRequesttogetCustomerAccount: " + responseBody);
